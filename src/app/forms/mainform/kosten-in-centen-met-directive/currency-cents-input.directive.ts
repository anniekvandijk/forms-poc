...
@Directive({
  selector: '[currencyCentsInput]',
  standalone: true,
  providers: [CurrencyPipe],
})
export class CurrencyCentsInputMaskDirective implements OnInit {
  ...
  ngOnInit() {
    ...
    //     this.el.value.replace(this.decimalSeparator, '.'),
    //     this.currencyCode,
    //   ) || '';
    // set initial value and format it to currency
    ...
    this.el.value =
    this.currencyPipe.transform(
      (parseInt(this.el.value)/100).toFixed(2),
      this.currencyCode,
    ) || '';
  }
  ...
  onBlur(value: any) {
    ...
    this.el.value =
      this.currencyPipe.transform(
        value.replace(this.decimalSeparator, '.'),
        this.currencyCode,
      ) || '';
  }
